...
def calculateBrokenSequenceFine():
    bracket_set = '(){}[]<>'
    opening = bracket_set[::2]
    closing = bracket_set[1::2]

    fine = [3, 1197, 57, 25137]
...
def calculateAddedBracketScore():
    ...
    with open('day10data.txt') as file:
        for sequence in file:
            ...
            for char in sequence.strip():
                ...
                while len(stack) > 0:
                    item = stack.pop()
                    localScore = localScore * 5 + fine[item]
                score.append(localScore)
    score.sort()

    return score[(len(score) - 1) // 2]
